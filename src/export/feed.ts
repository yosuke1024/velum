...
import { charPath, worldPath, feedRelPath, feedEntryName } from '../lib/paths.js';
...
import { both, oneLine, type Bilingual } from '../lib/bilingual.js';
...
export function buildCharactersFeed(
  ...
  const characters = CHARACTER_IDS.map((id) => {
    ...
    return {
      ...
      role: both(profile.role),
      age: profile.age,
      affiliation: profile.affiliation,
      intro: both(profile.intro),
      portrait: {
...
export const feedEntryId = (entry: DiaryEntry): string => `${entry.date}-${entry.protagonist}`;
...
export function feedEntryFrom(entry: DiaryEntry, body: Bilingual): FeedEntryFile {
  ...
  return FeedEntryFileSchema.parse({
    ...
    title: both(entry.title),
    // 一覧用抜粋。velum 内部の quote を転用する（契約 §1.2）。
    excerpt: both(entry.quote),
    mood: both(entry.mood),
...
export function collectFeedEntryFiles(): FeedEntryFile[] {
  ...
  for (const id of CHARACTER_IDS) {
    for (const path of listDatedFiles(charPath(id, 'entries'), '.json')) {
      ...
      if (!ja) {
        throw new Error(`${id} の ${entry.date} に日記本文（ja）がありません: ${path}`);
      }
